...
1. `>>>` 。如果项目使用的是css原生样式，那么可以直接使用`>>>`穿透样式。
...
#### (一) 编译入口
Vue3的编译入口通常是在vue/src/index.ts文件中，通过调用registerRuntimeCompiler将compile注入为编译方法，从而开始模板的编译过程。

#### (二)  模板编译流程

  **1. 模板字符串转换为AST：**    
  Vue3使用baseParse函数将模板字符串解析为抽象语法树（AST）。这个过程中，Vue3会利用正则表达式对模板字符串进行匹配、截取和解析，识别出标签、属性、插值表达式、注释等模板元素，并使用栈的数据结构来维护标签的父子层级关系。  

  **2. 对AST进行转换：**   
  在得到基础的AST后，Vue3会对其进行一系列的转换操作，以增加一些优化信息，如patchFlag等。这些转换操作有助于在后续的渲染过程中提高性能。  

  **3. 基于AST生成渲染函数：**   
  最后，Vue3会根据转换后的AST生成渲染函数。这个渲染函数是一个可执行的JavaScript函数，它接受上下文（如组件的实例、缓存等）作为参数，并返回一个虚拟DOM节点或节点树。这个虚拟DOM节点或节点树描述了组件的UI结构。  

#### (三)  编译优化
在Vue3的编译过程中，还包含了一些优化措施，以提高渲染性能和减少内存占用。例如：   
  + **静态提升：** Vue3会将一些静态的虚拟DOM节点提升到函数外部，以避免在每次渲染时都重新创建这些节点。这有助于减少内存分配和垃圾回收的开销。
  + **事件缓存：** 对于事件处理器，Vue3会将其缓存起来，以避免重复创建。这有助于减少内存占用并提高事件处理的性能。
  Patch标记：Vue3引入了patchFlag来标记不同类型的节点更新。这有助于在渲染过程中快速识别哪些节点需要更新，哪些节点可以保持不变，从而提高渲染性能。

### 什么是虚拟DOM？diff算法？
虚拟dom是用JavaScript语言描述的一个树形结构，用于缓存发生变化的响应式数据，当到达下次刷新周期的时候，一次性把所有发生变化的数据同步到实际dom上。diff算法是同步vdom和dom数据的算法，采用就地复用的策略实现高效同步。

### ref('')、ref()、ref(null)的区别？
ref('')的作用是封装普通数据，实现响应式数据；   
ref(null)和ref()的作用是获取子组件实例。
