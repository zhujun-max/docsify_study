...
![img](./img/other/44.png)



 ![img](./img/other/1icon.png) **—— “恢复（Resume）”：继续执行，快捷键 F8。**  
 继续执行。如果没有其他的断点，那么程序就会继续执行，并且调试器不会再控制程序。

 ![img](./img/other/2icon.png) **—— “下一步（Step）”：运行下一条（即当前行）指令，快捷键 F9。**

 ![img](./img/other/3icon.png) **—— “跨步（Step over）”：运行下一条（即当前行）指令，但 不会进入到一个函数中，快捷键 F10。**

 ![img](./img/other/4icon.png) **—— “步入（Step into）”，快捷键 F11。**

 ![img](./img/other/5icon.png) **—— “步出（Step out）”：继续执行到当前函数的末尾，快捷键 Shift+F11。**

 ![img](./img/other/6icon.png) **—— 启用/禁用所有的断点**
